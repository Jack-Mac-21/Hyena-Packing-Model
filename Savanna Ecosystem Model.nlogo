globals [
  lion-energy ;Used for energy graphs
  hyena-energy
  impala-energy
  antelope-energy
]
...
to go
  ask turtles [ ;consolidated the functions
    move
    eat
    reproduce
    check-death
...
to move ;Hyenas have a different move function
  rt random 90
  lt random 90
  if breed = lions [lion-move] ; seperate move function to control movement cost
  if breed = hyenas [hyena-move]
  if breed = antelopes [set energy (energy - movement-cost)]
  if breed = impalas [set energy (energy - movement-cost)]
  forward 1
end

to hyena-move ;They move randomly or towards another hyena
...
to lion-eat-procedure-3 ;Lions will prioritize impalas, but will eat a hyena given the chance
  let pack-count-number (count hyenas in-radius pack-radius) ;number of hyenas in the area
  if-else any? hyenas-here and (pack-count-number < 20)
  [
    let target one-of hyenas-here
    ask target [ die ]
    set energy (energy + lion-energy-gain-from-eating)
...
to impala-eat-procedure
  ...
  if ( grass-amount > 0 ) [
    ...
    set energy energy + (energy-gain-from-grass * grass-units-eaten-for-impala)
    ;; decrement the grass
...
to antelope-eat-procedure
  ...
  if ( grass-amount > 0 ) [
    ;; increment the sheep's energy
    set energy energy + (energy-gain-from-grass * grass-units-eaten-for-antelope)
    ;; decrement the grass
    set grass-amount grass-amount - grass-units-eaten-for-antelope
    recolor-grass
  ]
end
...
to regrow-grass ;grass regrows by the grass-regrowth-rate parameter
  ask patches [
...
;energy levels for reproduction can be modifed easily in each reproduce method
to reproduce ;different reproduction procedures are for different reproduction cost
...
to antelope-reproduce
  if energy > 130 [
    set energy energy - 40  ;; reproduction cost animal energy
...
to report-energy ; Used for tracking total energy levels
  set lion-energy 0
  set hyena-energy 0
  set impala-energy 0
...
to kill-lions ; In case you want to kill the lions
  ask lions [ die ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"Lion Energy" 1.0 0 -2674135 true "" "plot lion-energy"
"Hyena Energy" 1.0 0 -8431303 true "" "plot hyena-energy"
...
"" ""
PENS
"Hyenas" 1.0 0 -8431303 true "" "plot count hyenas"
"Lions" 1.0 0 -2674135 true "" "plot count lions"
"Antelopes" 1.0 0 -1184463 true "" "plot count antelopes"
"Impalas" 1.0 0 -7500403 true "" "plot count impalas"
...
724
535
grass-units-eaten-for-antelope
grass-units-eaten-for-antelope
0
5
1.0
...
This model is attempting to model an ecosystem of multiple animals, with a focus on the effects of animals packing together. This model was inspired by the African Savanna and hence the names of the animals are from animals located there. 

## HOW IT WORKS

Moving is done randomly and cost every agent a certain amount of energy. Antelopes and Impalas eat grass. Lions will eat Hyenas and Impalas, and if their energy is below the "preference-threshold" they will also eat antelopes. Hyenas can be modified to form packs by adjusting the "pack-radius" and "random-hyena-movement-probability." The former is the radius that hyenas will look for other hyenas to move towards and the ladder is the probability of moving randomly as opposed to moving towards random hyena within the radius.
 
## HOW TO USE IT

1. Modify the parameters as you prefer
2. Press setup to create the ecosystem
3. Press Go to start the simulation
4. Observe the behaviour in the window and in the graphs provided. 
5. The energy graph shows the total energy of the lions and the hyenas. You may find this    inciteful at times. 

PARAMETERS

LION PARAMETERS
lion-reproduction-threshold: Specifies the energy required for a lion to reproduce
preference-threshold: Specifies the energy level where a lion will begin eating antelopes
lion-energy-gain-from-eating: Amount of energy a lion gains from eating
lion-reproduction-cost: Amount of energy a lion loses from reproducing
lion-movement-cost: The amount energy a lion will lose per movement
preference-threshold?: Sets whether lions will prefer not to eat antelopes until they reach a low enough energy level

HYENA PARAMETERS
pack-radius: Sets the radius a hyena will search for another hyena to move to
hyena-energy-gain-from-eating: The amount of energy a hyena will gain from eating
hyena-movement-cost: The energy hyenas use to move
random-hyena-movement-probability: The chance a hyena will move randomly

IMPALA/ANTELOPE PARAMETERS 
grass-units-eaten-for-antelope: The amount of grass an antelope eat per eat
energy-gain-from-grass: The amount of energy in one unit of grass
movement-cost: energy required to move
grass-units-eaten-for-impala: The amount of grass units eaten per eat for impalas
grass-regrowth-rate: The amount grass regrows per tick


## THINGS TO TRY

1. Observe the difference between completely random hyena movement and setting random hyena movement to zero.

2. Investigate the changes in packing behaviour by shifting the pack-radius

3. Observe the changes that occur when the preference threshold for lions is deactivated

## EXTENDING THE MODEL

The preferences of lions and hyenas can be altered fairly easily and will yield new results.

Investigate what happens when reproduction is changed to a probability based on the energy level.

See how changing the value required for hyenas to kill impalas and lion effects populations. 


## RELATED MODELS

Similar to other predator prey models. 

## CREDITS AND REFERENCES 

Nicholls, Henry. “Earth - The Truth about Spotted Hyenas.” BBC. BBC, October 28, 2014. http://www.bbc.com/earth/story/20141028-the-truth-about-spotted-hyenas.
